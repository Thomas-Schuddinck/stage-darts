    ...
    <Aux>
      <Grid container className={classes.lijst}>
        {props.playerleg.turns.map(function (s: Turn, i: any) {
          if (props.klein === true){
            return <Grid item xs={6} md={6} lg={4} key={"klei-grid-" + i}><TurnComponent key={i} beurt={i + 1} turn={s} /></Grid>
          }else{
            return <Grid item xs={6} md={2} lg={2} key={"big-grid-" + i}><TurnComponent key={i} beurt={i + 1} turn={s} /></Grid>
          }
